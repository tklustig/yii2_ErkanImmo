                        ...
                        <div id="collapseOne" class="panel-collapse collapse in"> <!-- !weist der Column die JS-Id zu!-->
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    $counter = 1;
                                    foreach ($Ids as $item) {
                                        if ($model::getBild($item) != NULL) {
                                            $pic = $model::getBild($item);
                                            $Pic2Show = Html::img($pic, ['alt' => 'Kundenbild nicht vorhanden', 'class' => 'img-circle', 'style' => 'width:75px;height:75px']);
                                        } else
                                            $Pic2Show = null;
                                        ?>
                                        <div><label style='font-weight: lighter'><?= $counter . ': ' . $geschlecht[$counter - 1] . ' ' ?><?= $name[$counter - 1] ?></label></div>
                                        <?php
                                        if (!empty($Pic2Show))
                                            echo $Pic2Show;
                                        else {
                                            ?><div><label>Kundenbild nicht hinterlegt</label></div>
                                            <?php } ?>
                                        <label><font color='blue'><?= $Mailadress[$counter - 1] ?></font></label>
                                        <?php
                                        $counter++;
                                    }
                                    ?>
